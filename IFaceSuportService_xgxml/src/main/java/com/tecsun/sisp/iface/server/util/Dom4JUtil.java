...
public class Dom4JUtil {
	...
	public static void weather(){
		...
		try {   
			String url = Config.getInstance().get("XIANGANWEATHER");
			 
			SAXReader reader = new SAXReader();   
			Document doc = reader.read(url);   
			Element root = doc.getRootElement();   
			Element foo;   
			for (Iterator i = root.elementIterator("city "); 
					i.hasNext();) {
				foo = (Element) i.next();   
				System.out.print("城市编号:" + foo.elementText("cityY"));   
				System.out.println("城市:" + foo.elementText("cityname"));
				System.out.println("天气:"+foo.elementText("windState"));
				}   
} catch (Exception e) {   
			e.printStackTrace();   
	    }   
