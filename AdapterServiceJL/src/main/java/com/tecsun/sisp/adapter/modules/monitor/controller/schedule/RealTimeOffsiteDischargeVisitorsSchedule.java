...
/**
 * 近24小时交易信息---异地就医出院人次统计数据写入redis
 */
public class RealTimeOffsiteDischargeVisitorsSchedule {
	
	private static Logger logger = LoggerFactory.getLogger(RealTimeOffsiteDischargeVisitorsSchedule.class);

	@Autowired
	private MonitorServiceImpl monitorServiceImpl;
	
	
	public void saveRealTimeOffsiteDischargeVisitorsData() {
		
		logger.info("....定时任务.....近24小时交易信息异地就医出院人次统计数据写入redis.....开始执行.......");
		...
		// compareDatas添加数据，compareDatas默认里面各时间段（0~23）的数据都为0
		for (int index = 0; index < 24; index++) {
			compareDatas.add(index, new RealTimeDischargeVisitorsVO("" + index, 0));
		}
		
		try {
			
			// 清空缓存过期数据
			JedisUtil.delValue("offsiteRealTimeDischarge");
			datas = monitorServiceImpl.getRealTimeOffsiteDischargeVisitors4Other(queryBean);
			
			// datas与compareDatas取并集，将某些时间段没有数据就置为0，最终所有的数据全部存储在compareDatas
			...
			logger.info("********近24小时交易信息异地就医出院人次统计数据 " + jsonData + "********");
			JedisUtil.setValue("offsiteRealTimeDischarge", jsonData, 24*60);
			
			long finishTime = DateUtil.currentSeconds();
			logger.info(".........近24小时交易信息异地就医出院人次统计数据成功写入redis.........");
			logger.info(".........近24小时交易信息异地就医出院人次统计数据写入redis耗时 " + (finishTime - beginTime) + " 秒.........");
			
		} catch (Exception e) {
			logger.info("近24小时交易信息异地就医出院人次统计数据写入redis异常", e);
		}
