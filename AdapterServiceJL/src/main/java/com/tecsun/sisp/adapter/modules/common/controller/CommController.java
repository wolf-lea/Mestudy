...
import com.tecsun.sisp.adapter.modules.common.entity.response.*;
...
public class CommController extends BaseController {
    ...
    /**
     * 人脸对比(两张照片)认证业务：001-实名支付认证；003-社保卡申请；004-补换卡申请；010-国政通临时；020-刷脸登录认证
     * 将认证图片关联业务、人员
     * 将比对图片图片关联业务、人员
    ...
    public Result comparePhoto(@RequestBody VerifyBean bean) throws Exception {
        ...
        if (!"020".equals(bean.getVerifyBus()) && !"010".equals(bean.getVerifyBus()) && bean.getComparisonData() == 0) {
            return this.result(Constants.RESULT_MESSAGE_PARAM, "参数不全");
        }
        String statusCode = Constants.RESULT_MESSAGE_ERROR;
        String message = "操作失败";
        VerifyResult vResult = new VerifyResult();
        AccountVO accountVO = new AccountVO();//用于刷脸登录返回个人信息
        ...
        try {
            ...
            if ("020".equals(bean.getVerifyBus())) {
                //认证类型为刷脸登录认证
                map = FaceIface.onlineauth(bean.getSfzh(), picPath, bean.getVerifyChannel());
            }else if (!"010".equals(bean.getVerifyBus())) {
                //比对图片信息
            ...
            if (Constants.RESULT_MESSAGE_SUCCESS.equals(statusCode)) {
                ...
                //当认证类型为刷脸登录认证 获取个人信息
                if ("020".equals(bean.getVerifyBus())) {
                    List<AccountVO> list = accountService.getAccountInfo4Cssp(null, bean.getSfzh(), null);
                    if (list != null && !list.isEmpty()) {
                        accountVO = list.get(0);
...
//            if(StringUtils.isNotEmpty(map.get("LONGITUDE"))  && StringUtils.isNotEmpty(map.get("LATITUDE"))) {
            if(map != null) {
                ...
                coord.put("latitude",map.get("LATITUDE"));
    ...
    @RequestMapping(value = "/getNotice", method = RequestMethod.POST, produces = "application/json")
    @ResponseBody
    public Result getNotice(@RequestBody NoticeBean bean){
        //入参校验
        if(StringUtils.isEmpty(bean.getCode())){
            return this.result(Constants.RESULT_MESSAGE_PARAM, "请输入所属编码");
        }
        try {
            NoticeVo noticeVo = commService.getNotice4cssp(bean);
            return ok("查询成功", noticeVo);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("common-getNotice报错：" + e);
        }
        return error("获取信息失败");
    }
     ...
     * @return
     */
    @RequestMapping(value = "/getDistinceByParentCode", method = RequestMethod.POST, produces = "application/json")
    @ResponseBody
    public Result getDistinceByParentCode(@RequestBody AreaQueryBean bean){
        //入参校验
        if(StringUtils.isEmpty(bean.getAreaCode())){
            return this.result(Constants.RESULT_MESSAGE_PARAM, "请输入父级区域编码");
        }
        try {
        	List<AreaInfoVo> list = commService.getDistinceByParentCode4Sisp(bean);
            return ok("查询成功", list);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("common-getDistinceByParentCode报错：" + e);
        }
        return error("获取信息失败");
    }
     ...
     * @return
     */
    @RequestMapping(value = "/getAllServiceBlank", method = RequestMethod.POST, produces = "application/json")
    @ResponseBody
    public Result getAllServiceBlank(@RequestBody AreaQueryBean bean){
        //入参校验
       
        try {
        	List<ServiceBlankVO> list = commService.getAllServiceBlank4Sisp();
            return ok("查询成功", list);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("common-getAllServiceBlank报错：" + e);
        }
        return error("获取信息失败");
    }
     ...
     * @return
     */
    @RequestMapping(value = "/getBlankNetList", method = RequestMethod.POST, produces = "application/json")
    @ResponseBody
    public Result getBlankNetList(@RequestBody BranchBean bean){
        //入参校验
       
        try {
        	List<ServiceBlankVO> list = commService.getBlankNetList4Sisp(bean);
            return ok("查询成功", list);
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("common-getAllServiceBlank报错：" + e);
