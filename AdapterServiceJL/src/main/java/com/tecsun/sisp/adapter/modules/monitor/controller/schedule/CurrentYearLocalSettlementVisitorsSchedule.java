...
/**
 * 当年就医统计---本地就医结算人次统计数据写入redis
 */
public class CurrentYearLocalSettlementVisitorsSchedule {
	
	private static Logger logger = LoggerFactory.getLogger(CurrentYearLocalSettlementVisitorsSchedule.class);

	@Autowired
	private MonitorServiceImpl monitorServiceImpl;
	
	
	public void saveCurrentYearLocalSettlementVisitorsData() {
		
		// 当年时间
		String currentYear = DateUtil.format(new Date(), "yyyy");
		logger.info("....定时任务....." + currentYear + "年本地就医结算人次统计数据写入redis.....开始执行.......");
		...
		try {
			JedisUtil.delValue("localSettlementStatistics");
			// 获取统计数据
			datas = monitorServiceImpl.getCurrentYearLoclaSettlementVisitors4Other(queryBean);
			...
			if (CollUtil.isNotEmpty(compareDatas)) {
				...
				logger.info("********" + currentYear + "年本地就医结算人次统计数据" + jsonData + "********");
				JedisUtil.setValue("localSettlementStatistics", jsonData, 24*60);
			}
			
			long endTime = DateUtil.currentSeconds();
			logger.info("........." + currentYear + "年本地就医结算人次统计数据成功写入redis.........");
			logger.info("........." + currentYear + "年本地就医结算人次统计数据写入redis耗时" + (endTime - startTime) + "秒.........");
			
		} catch (Exception e) {
			logger.info(currentYear + "年本地就医结算人次统计数据写入redis异常", e);
		}
