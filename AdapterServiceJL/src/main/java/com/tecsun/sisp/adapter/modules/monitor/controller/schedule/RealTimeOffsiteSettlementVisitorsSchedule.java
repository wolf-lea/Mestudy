...
/**
 * 近24小时交易信息---异地就医结算人次统计数据写入redis 
 */
public class RealTimeOffsiteSettlementVisitorsSchedule {
	...
	public void saveRealTimeOffsiteSettlementVisitorsData() {
		
		logger.info("....定时任务.....近24小时交易信息异地就医结算人次统计数据写入redis.....开始执行.......");
		...
		try {
			// 清除缓存中过期数据
			JedisUtil.delValue("offsiteRealTimeSettlement");
			datas = monitorServiceImpl.getRealTimeOffsiteSettlementVisitors4Other(queryBean);
			...
			logger.info("********近24小时交易信息异地就医结算人次统计数据" + jsonData + "********");
			JedisUtil.setValue("offsiteRealTimeSettlement", jsonData, 24*60);
			
			long finishTime = DateUtil.currentSeconds();
			logger.info(".........近24小时交易信息异地就医结算人次统计数据成功写入redis.........");
			logger.info(".........近24小时交易信息异地就医结算人次统计数据写入redis耗时 " + (finishTime - beginTime) + " 秒.........");
			
		} catch (Exception e) {
			logger.info("近24小时交易信息异地就医结算人次统计数据写入redis异常", e);
		}
