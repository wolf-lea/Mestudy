 ...
 * 银行返回报文处理器 :64域
 * 如建行
 * copy RCCBankPkgDecodeHandler update processAllBody method
 * @author tan
 *
 */
public class CCBBankPkgDecodeHandler extends SimpleChannelHandler {
    public static final Logger logger = Logger.getLogger(CCBBankPkgDecodeHandler.class);

    // 报文定义的 报文长度字节数
    private static final Integer BODYLENBYTESC = 2;
    short bodyLen = 0;
    ...
    public CCBBankPkgDecodeHandler(byte[] reqBs, HandlerResult result) {
        this.reqBs = reqBs;
        this.result = result;
    }
    ...
    public void messageReceived(ChannelHandlerContext ctx, MessageEvent e) throws Exception {
        ...
        logger.info(">>>>>>>>>>>> 接收到 银行（64域） 的响应报文-Response byteSize:" + msg.readableBytes());
        // 合并字节
        ...
        if (!isGetBodyLen) {
            if (cacheBuffer.readableBytes() >= BODYLENBYTESC) {
                bodyLen = cacheBuffer.readShort();
                byte[] bs =BKUtils.short2Byte(bodyLen);
    ...
    private void processAllBody(ChannelBuffer msg, Channel channel) {
        ...
        logger.info("********* 响应报文（64域）bank return bytes hexStr=" +hexStr);
