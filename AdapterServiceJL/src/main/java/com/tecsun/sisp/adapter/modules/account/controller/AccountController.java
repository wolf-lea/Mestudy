        ...
        try {
            if (bean.getAccountPwd().equals(bean.getReAccountPwd())) {
                ...
                if (listSize == 0 && StringUtils.isNotEmpty(bean.getOpenid())) {
                    list = accountService.getAccountInfoByOpenid4Cssp(bean.getOpenid(),"1");
                    if (list != null && !list.isEmpty()) {
                        message = "该微信号已被其他账号绑定";
                        listSize = list.size();
                    }else{
                    	List<AccountVO> lists = accountService.getAccountInfoByOpenid4Cssp(bean.getOpenid(),"0");
                    	listSize = lists.size();
                    }
                }
                ...
                if (listSize == 0) {
                    ...
                    if (roleVOs == null || roleVOs.isEmpty()) {
                        ...
                        if (addStatus > 0) {
                        	if("0".equals(bean.getIsWechat())){
                        	   message = "人脸认证不通过，注册失败！";
                        	 }else{
                               message = "注册成功";
                    ...
                    if ("0".equals(bean.getStatus())) {
                        ...
                        if (Constants.WECHAT.equals(bean.getChannelcode()) && StringUtils.isNotBlank(bean.getOpenid())) {
                            if (StringUtils.isNotBlank(list.get(0).getOpenid())&&"1".equals(list.get(0).getIsWechat())) {
    ...
    public Result getAccountInfo(@RequestBody AccountBean bean) throws Exception {
        ...
        try {
            if (StringUtils.isNotEmpty(bean.getOpenid()))
                ...
                list = accountService.getAccountInfoByAccountId4Cssp(bean.getAccountId(),"");
                String pic = accountService.selectAccountPhoto4Cssp(bean);
                if(pic != null){
                    String picPath= commService.photoIsExist4Cssp(Long.parseLong(pic),Constants.PICTURE_TYPE_003);
                    list.get(0).setPic((ImageChangeUtil.getImageStr(picPath)));
                }
            }
            if (list != null && !list.isEmpty()) {
    ...
    public Result saveAccountInfo(@RequestBody AccountBean bean) throws Exception {
        ...
        try {
            ...
            if (accountVOList != null && !accountVOList.isEmpty()) {
                ...
                if (listSize == 0) {
                    //上传个人信息头像
                    if(StringUtils.isNotBlank(bean.getAccountId()) && StringUtils.isNotBlank(bean.getPic())){
                        // 把图片存到公服T_YTH_PICTURE_INFO表
                        PicBean picBean=new PicBean();
                        picBean.setPicBase64(bean.getPic());
                        picBean.setPicType(Constants.PICTURE_TYPE_003);
                        long picId = commService.uploadPicture4Cssp(picBean);
                        if(picId==-1) return this.result(Constants.RESULT_MESSAGE_PARAM, "入参不正确，传入的图像信息有误");
                            ...
                            if(commService.insertPictureBus4Cssp(picBusBean)>0){
                                bean.setPic(String.valueOf(picBean.getPicId()));
                                accountService.updateAccountPhoto4Cssp(bean);
                            }
                        }
                    }
