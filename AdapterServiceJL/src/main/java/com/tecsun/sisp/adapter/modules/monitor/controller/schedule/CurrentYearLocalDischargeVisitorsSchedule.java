...
/**
 * 当年就医统计---本地就医出院人次统计数据写入redis
 */
public class CurrentYearLocalDischargeVisitorsSchedule {
	
	private static Logger logger = LoggerFactory.getLogger(CurrentYearLocalDischargeVisitorsSchedule.class);

	@Autowired
	private MonitorServiceImpl monitorServiceImpl;
	
	
	public void saveCurrentYearLocalDischargeVisitorsData() {
		
		// 当年时间
		String currentYear = DateUtil.format(new Date(), "yyyy");
		logger.info("....定时任务....." + currentYear + "年本地就医出院人次统计数据写入redis.....开始执行.......");
		...
		try {
			JedisUtil.delValue("localDischargeStatistics");
			// 获取统计数据
			datas = monitorServiceImpl.getCurrentYearLocalDischargeVisitors4Other(queryBean);
			...
			if (CollUtil.isNotEmpty(compareDatas)) {
				...
				logger.info("********" + currentYear + "年本地就医出院人次统计数据" + jsonData + "********");
				JedisUtil.setValue("localDischargeStatistics", jsonData, 24*60);
			}
			
			long endTime = DateUtil.currentSeconds();
			logger.info("........." + currentYear + "年本地就医出院人次统计数据成功写入redis.........");
			logger.info("........." + currentYear + "年本地就医出院人次统计数据写入redis耗时" + (endTime - startTime) + "秒.........");
			
		} catch (Exception e) {
			logger.info(currentYear + "年本地就出院人次统计数据写入redis异常", e);
		}
	}
