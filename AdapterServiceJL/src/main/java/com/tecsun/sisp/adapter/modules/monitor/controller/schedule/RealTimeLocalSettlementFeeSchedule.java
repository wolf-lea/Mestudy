...
/**
 * 近24小时交易信息---本地就医结算费用统计数据写入redis 
 */
public class RealTimeLocalSettlementFeeSchedule {

	private static Logger logger = LoggerFactory.getLogger(RealTimeLocalSettlementFeeSchedule.class);

	@Autowired
	private MonitorServiceImpl monitorServiceImpl;
	
	
	public void saveRealTimeLocalSettlementFeeData() {
		...
		try {
			// 清除缓存中过期数据
			JedisUtil.delValue("localRealTimeSettlementFee");
			datas = monitorServiceImpl.getRealTimeLocalSettlementFee4Other(queryBean);
			...
			logger.info("********近24小时交易信息本地就医结算费用统计数据" + jsonData + "********");
			JedisUtil.setValue("localRealTimeSettlementFee", jsonData, 24*60);
			
			long finishTime = DateUtil.currentSeconds();
			logger.info(".........近24小时交易信息本地就医结算费用统计数据成功写入redis.........");
			logger.info(".........近24小时交易信息本地就医结算费用统计数据写入redis耗时" + (finishTime - beginTime) + "秒.........");
			
		} catch (Exception e) {
			logger.info("近24小时交易信息本地就医结算费用统计数据写入redis异常", e);
		}
