...
import com.tecsun.sisp.adapter.common.util.treatment.FingerPrintIface;
...
import com.tecsun.sisp.adapter.modules.treatment.entity.response.CollIntoVO;
...
@Controller
@RequestMapping(value = "/adapter/fingerprint")
public class FingerPrintController  extends BaseController {
     ...
     * @return
     */
    @RequestMapping(value = "/isCollectFingerPrint", method = RequestMethod.POST, produces = "application/json")
    @ResponseBody
    public Result isCollectFingerPrint(@RequestBody SecQueryBean bean) throws Exception {
        ...
        String message = "操作失败";
        try {
            Map map = FingerPrintIface.isCollectFingerprint(bean.getSfzh(), bean.getXm());
            statusCode = String.valueOf(map.get("statusCode"));
            message = String.valueOf(map.get("message"));
        } catch (Exception e) {
            logger.error("待遇资格认证-指纹是否已采集异常：", e);
        }
     ...
     * 某些项目指纹采集需传头像，待完善从第三方获取头像方法
     * 个人信息头像来源： 查看认证云采集数据接口（将建模照片充当头像）,如果认证云未采集，则头像为第三方来源基本信息中的头像
     * @param bean
     * @return
     */
    @RequestMapping(value = "/fingerPrintCollection", method = RequestMethod.POST, produces = "application/json")
    @ResponseBody
    public Result fingerPrintCollection(@RequestBody CollectBean bean) throws Exception {
        if (StringUtils.isBlank(bean.getSfzh()) || StringUtils.isBlank(bean.getXm())
        ...
        if (bean.getColData() == 0 && StringUtils.isBlank(bean.getColBase64())) {
            return this.result(Constants.RESULT_MESSAGE_PARAM, "指纹采集数据不全");
        }
        String statusCode = Constants.RESULT_MESSAGE_ERROR;
        String message = "操作失败";
...
/*
            ...
            */
            ...
            String picType = Constants.PICTURE_TYPE_205;
            if ("1".equals(bean.getColHand())) picType = Constants.PICTURE_TYPE_204;
            //如果指纹图片不为空，则查询图片相关信息
            String picPath = null;
            ...
            //指纹采集
            Map map = FingerPrintIface.collectFingerprint(list.get(0), bean.getColBase64(), personPicPhoto, picPath);
            statusCode = String.valueOf(map.get("statusCode"));
            message = String.valueOf(map.get("message"));
            ...
            try {
                ...
                statusCode = Constants.RESULT_MESSAGE_ERROR;
                message = "采集失败";
                logger.error("待遇资格认证-指纹采集插入数据库错误：", e);
            }
        }
        return result(statusCode, message);
    }
...
//   ----------------- 认证 --------------------------------
     ...
     * @return
     */
    @RequestMapping(value = "/fingerPrintVerification", method = RequestMethod.POST, produces = "application/json")
    @ResponseBody
    public Result fingerPrintVerification(@RequestBody VerifyBean bean) throws Exception {
        ...
        if (bean.getVerifyData() == 0 && StringUtils.isBlank(bean.getVerifyBase64())) {
            return this.result(Constants.RESULT_MESSAGE_PARAM, "指纹认证数据不全");
        }
        String statusCode = Constants.RESULT_MESSAGE_ERROR;
        String message = "操作失败";
        ...
        try {
            ...
            String picType = Constants.PICTURE_TYPE_206;
            //如果指纹图片不为空，则查询图片相关信息
            ...
            //指纹认证
            Map map = FingerPrintIface.fingerprintAuth(bean);
            statusCode = String.valueOf(map.get("statusCode"));
            message = String.valueOf(map.get("message"));
            if (Constants.RESULT_MESSAGE_SUCCESS.equals(statusCode)) {
            ...
            message = "认证失败";
            bean.setVerifyResult(Constants.PIXEL_BUSINESS_STATUS_02);
            logger.error("待遇资格认证-指纹认证(云端)异常：", e);
        } finally {
            try {
                ...
                statusCode = Constants.RESULT_MESSAGE_ERROR;
                message = "认证失败";
                logger.error("待遇资格认证-指纹认证(云端)插入数据库错误：", e);
            }
        }
     ...
     * @return
     */
    @RequestMapping(value = "/fingerPrintOnlineAuth", method = RequestMethod.POST, produces = "application/json")
    @ResponseBody
    public Result fingerPrintOnlineAuth(@RequestBody VerifyBean bean) throws Exception {
        if (StringUtils.isBlank(bean.getSfzh()) || StringUtils.isBlank(bean.getXm())
        ...
        try {
            ...
            //认证云指纹1：1比对
            Map map = FingerPrintIface.fingerOnlineAuth(bean);
            statusCode = String.valueOf(map.get("statusCode"));
            message = String.valueOf(map.get("message"));
            if (Constants.RESULT_MESSAGE_SUCCESS.equals(statusCode)) {
                bean.setVerifyResult(Constants.PIXEL_BUSINESS_STATUS_01);
            ...
            try {
                ...
                statusCode = Constants.RESULT_MESSAGE_ERROR;
                message = "比对失败";
                logger.error("指纹对比/1:1认证插入数据库错误：", e);
            }
        }
        return result(statusCode, message, vResult);
    }
    ...
    /**
     * 待遇资格认证-获取指纹采集信息（终端认证）
     * 废弃
     * @param bean
     * @return
     */
    @RequestMapping(value = "/getFingerPrintData", method = RequestMethod.POST, produces = "application/json")
    @ResponseBody
    public Result getFingerPrintData(@RequestBody CollectBean bean) throws Exception {
        if (StringUtils.isBlank(bean.getSfzh()) || StringUtils.isBlank(bean.getXm())) {
            return this.result(Constants.RESULT_MESSAGE_PARAM, "参数不全");
        }
        String statusCode = Constants.RESULT_MESSAGE_ERROR;
        ...
        try {
            ...
            Map map = DictionaryUtil.rzyPostClient(String.valueOf(jsonObject), RZY_URL + Constants.RZY_GETFINGERPRINTDATA);
            statusCode = String.valueOf(map.get("statusCode"));
            message = String.valueOf(map.get("message"));
        } catch (Exception e) {
            logger.error("待遇资格认证-获取指纹采集信息异常：", e);
            statusCode = Constants.RESULT_MESSAGE_ERROR;
            message = "操作失败";
        }
     ...
     * @return
     */
    @RequestMapping(value = "/uploadFingerPrintVerify", method = RequestMethod.POST, produces = "application/json")
    @ResponseBody
    public Result uploadFingerPrintVerify(@RequestBody VerifyBean bean) throws Exception {
        if (StringUtils.isBlank(bean.getSfzh()) || StringUtils.isBlank(bean.getXm()) || bean.getVerifyId() == 0
        ...
        try {
            ...
            //保存指纹认证结果
            Map map = FingerPrintIface.saveFingerPrintVerifyResult(bean);
            statusCode = String.valueOf(map.get("statusCode"));
            message = String.valueOf(map.get("message"));
            ...
            if ("1".equals(bean.getVerifyResult()) && Constants.RESULT_MESSAGE_SUCCESS.equals(statusCode)) {
                ...
                if ("0".equals(bean.getVerifyResult()))
                    bean.setFailReason(StringUtils.isEmpty(bean.getFailReason()) ? "前端指纹认证失败" : bean.getFailReason());
            ...
            logger.error("待遇资格认证-上传指纹认证结果（终端）异常：", e);
        } finally {
            try {
                ...
                statusCode = Constants.RESULT_MESSAGE_ERROR;
                message = "提交失败";
                logger.error("待遇资格认证-上传指纹认证结果（终端）修改数据库错误：", e);
            }
        }
        return result(statusCode, message, vResult);
    }
