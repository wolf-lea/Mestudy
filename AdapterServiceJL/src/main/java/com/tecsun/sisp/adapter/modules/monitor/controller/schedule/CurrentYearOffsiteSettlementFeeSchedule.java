...
/**
 * 当年就医结算---异地就医结算费用统计数据写入redis
 */
public class CurrentYearOffsiteSettlementFeeSchedule {

	private static Logger logger = LoggerFactory.getLogger(CurrentYearOffsiteSettlementFeeSchedule.class);

	@Autowired
	private MonitorServiceImpl monitorServiceImpl;
	
	
	public void saveCurrentYearOffsiteSettlementFeeData() {
		
		// 当年时间
		String currentYear = DateUtil.format(new Date(), "yyyy");
		logger.info("....定时任务....." + currentYear + "年异地就医结算费用统计数据写入redis.....开始执行.......");
		...
		try {
			JedisUtil.delValue("offsiteSettlementFeeStatistics");
			// 获取数据
			datas = monitorServiceImpl.getCurrentYearOffsiteSettelmentFee4Other(queryBean);
			
			// compareDatas与datas取并集，将某些月份没有数据手动置为0
			...
			logger.info("********" + currentYear + "年异地就医结算费用统计数据" + jsonData + "********");
			JedisUtil.setValue("offsiteSettlementFeeStatistics", jsonData, 24*60);
			
			long endTime = DateUtil.currentSeconds();
			logger.info("........." + currentYear + "年异地就医结算费用统计数据成功写入redis.........");
			logger.info("........." + currentYear + "年异地就医结算费用统计数据写入redis耗时" + (endTime - startTime) + "秒.........");
			
		} catch (Exception e) {
			logger.info(currentYear + "年异地就医结算费用统计数据写入redis异常", e);
		}
