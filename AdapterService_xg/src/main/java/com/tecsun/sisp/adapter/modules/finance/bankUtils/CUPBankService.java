...
import com.tecsun.sisp.adapter.modules.finance.bankUtils.handler.CUPBankPkgDecodeHandler;
...
/**
 * 此类用于 中国银联 的请求交易(128域报文)
 * Created by Ivan on 15-6-19.
 */
public class CUPBankService {
    public static final Logger logger = Logger.getLogger(CUPBankService.class);
    ...
    public CUPBankService(String ip, int port, int timeOut) {
        this.ip = ip;
        this.port = port;
    ...
    public HandlerResult sendReq(final byte[] reqBs) {
        ...
        bootstrap.setPipelineFactory(new ChannelPipelineFactory() {
            public ChannelPipeline getPipeline() {
                return Channels.pipeline(new CUPBankPkgDecodeHandler(reqBs, result));
            }
        });
