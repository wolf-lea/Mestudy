...
import com.tecsun.sisp.adapter.modules.common.entity.request.AreaQueryBean;
import com.tecsun.sisp.adapter.modules.common.entity.request.BranchBean;
...
public class CommServiceImpl extends BaseService {
	...
	public Page<AreaInfoVo> getAreaList4Sisp(IneQueryBean ineQueryBean) {
		Map<String,String> map = new HashMap<>();
		Page<AreaInfoVo> page = new Page<>(ineQueryBean.getPageno(),ineQueryBean.getPagesize());
		...
		if(list.size()>0){
			...
			list = this.getSqlSessionTemplate().selectList(NAMESPACE + "getAreaInfo", map);
			if(list.size()>0){
				page.setData(list);
				page.setCount(list.size());
			}
