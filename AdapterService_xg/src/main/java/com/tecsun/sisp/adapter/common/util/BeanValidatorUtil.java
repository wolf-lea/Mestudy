...
public class BeanValidatorUtil {
	...
	private static Logger logger = LoggerFactory.getLogger(BeanValidatorUtil.class);
	...
	public static void getFieldGetterNames(Object o) {
		...
		Map<String, Class> map = BeanValidatorUtil.getClassFields(o.getClass(), true);
		for (Map.Entry<String, Class> entry : map.entrySet()) {
			...
			String value = String.valueOf(getFieldValueByName(key, o, true));
	 ...
	 * 根据属性名获取属性值
	 * */
	private static Object getFieldValueByName(String fieldName, Object o, boolean includeParentClass) {
		try {
	...
	private static Map getFiledName(Class clazz, boolean includeParentClass) {
		...
		if (includeParentClass) {
			Map fieldNamesTemp = getFiledName(clazz.getSuperclass(), includeParentClass);
	...
	private static List getFiledsInfo(Object o) {
		...
		for (int i = 0; i < fields.length; i++) {
			...
			infoMap.put("value", getFieldValueByName(fields[i].getName(), o, false));
			list.add(infoMap);
		}
		return list;
	}
