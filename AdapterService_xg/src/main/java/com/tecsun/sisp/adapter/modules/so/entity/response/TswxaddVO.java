package com.tecsun.sisp.adapter.modules.so.entity.response;
/*
@author   hudanmeng
@date     2016-4-5
@ 五险补充查询VO
*/
public class TswxaddVO {
    private String  grbh; //个人编号
    private String  gsfssj; //工伤发生时间
    private String  shbw; //受伤部位
    private String  shdd; //受伤地点
    private String  rdjl; //认定结论
    private String  rdrq; //认定日期
    private String  ksny; //开始年月
    private String  zzny; //终止年月
    private String  gsdyje; //工伤待遇金额
    private String  zje; //总金额
    private String  scdjjdrq;//伤残等级鉴定日期
    private String  scdj;//伤残等级
    private String  hldjjdrq;//护理等级鉴定日期
    private String  hldj;//护理等级
    private String  dylb; //待遇类别
    private String  jsq; //结算期
    private String  shje; //审核金额
    private String  zfzt; //支付状态
    private String  syfssj; //生育发生时间
    private String  sybxdylb; //生育保险待遇类别
    private String  sbfy; //申报费用
    private String  sybxdyje; //生育保险待遇金额
    private  String ylffsze;//医疗费发生总额
    private String bxje;//报销金额
    private String yhzh;//银行账号

    public String getBxje() {
        return bxje;
    }

    public void setBxje(String bxje) {
        this.bxje = bxje;
    }

    public String getYlffsze() {
        return ylffsze;
    }

    public void setYlffsze(String ylffsze) {
        this.ylffsze = ylffsze;
    }

    public String getYhzh() {
        return yhzh;
    }

    public void setYhzh(String yhzh) {
        this.yhzh = yhzh;
    }

    public String getHldj() {
        return hldj;
    }

    public void setHldj(String hldj) {
        this.hldj = hldj;
    }

    public String getScdjjdrq() {
        return scdjjdrq;
    }

    public void setScdjjdrq(String scdjjdrq) {
        this.scdjjdrq = scdjjdrq;
    }

    public String getScdj() {
        return scdj;
    }

    public void setScdj(String scdj) {
        this.scdj = scdj;
    }

    public String getHldljdrq() {
        return hldjjdrq;
    }

    public String getHldjjdrq() {
        return hldjjdrq;
    }

    public void setHldjjdrq(String hldjjdrq) {
        this.hldjjdrq = hldjjdrq;
    }

    public String getGrbh() {
        return grbh;
    }

    public void setGrbh(String grbh) {
        this.grbh = grbh;
    }

    public String getGsfssj() {
        return gsfssj;
    }

    public void setGsfssj(String gsfssj) {
        this.gsfssj = gsfssj;
    }

    public String getShbw() {
        return shbw;
    }

    public void setShbw(String shbw) {
        this.shbw = shbw;
    }

    public String getShdd() {
        return shdd;
    }

    public void setShdd(String shdd) {
        this.shdd = shdd;
    }

    public String getRdjl() {
        return rdjl;
    }

    public void setRdjl(String rdjl) {
        this.rdjl = rdjl;
    }

    public String getRdrq() {
        return rdrq;
    }

    public void setRdrq(String rdrq) {
        this.rdrq = rdrq;
    }

    public String getKsny() {
        return ksny;
    }

    public void setKsny(String ksny) {
        this.ksny = ksny;
    }

    public String getZzny() {
        return zzny;
    }

    public void setZzny(String zzny) {
        this.zzny = zzny;
    }

    public String getGsdyje() {
        return gsdyje;
    }

    public void setGsdyje(String gsdyje) {
        this.gsdyje = gsdyje;
    }

    public String getZje() {
        return zje;
    }

    public void setZje(String zje) {
        this.zje = zje;
    }

    public String getDylb() {
        return dylb;
    }

    public void setDylb(String dylb) {
        this.dylb = dylb;
    }

    public String getJsq() {
        return jsq;
    }

    public void setJsq(String jsq) {
        this.jsq = jsq;
    }

    public String getShje() {
        return shje;
    }

    public void setShje(String shje) {
        this.shje = shje;
    }

    public String getZfzt() {
        return zfzt;
    }

    public void setZfzt(String zfzt) {
        this.zfzt = zfzt;
    }

    public String getSyfssj() {
        return syfssj;
    }

    public void setSyfssj(String syfssj) {
        this.syfssj = syfssj;
    }

    public String getSybxdylb() {
        return sybxdylb;
    }

    public void setSybxdylb(String sybxdylb) {
        this.sybxdylb = sybxdylb;
    }

    public String getSbfy() {
        return sbfy;
    }

    public void setSbfy(String sbfy) {
        this.sbfy = sbfy;
    }

    public String getSybxdyje() {
        return sybxdyje;
    }

    public void setSybxdyje(String sybxdyje) {
        this.sybxdyje = sybxdyje;
    }
}
