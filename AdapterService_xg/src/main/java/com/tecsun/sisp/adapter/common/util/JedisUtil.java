...
/**
 * ClassName: JedisUtil Description: Author： 张清洁 CreateTime： 2015年06月07日 16时:00分
 */
public class JedisUtil {
	...
	public static void setValue(String key, String value) throws Exception {
		setValue(key, value,
				Integer.parseInt(Config.getInstance().get("redis_iface_expx")));
	}
	...
	//redis永久
    public static void setValueLong(String key, String value) throws Exception {
        JedisPool publicJedisPool = null;
        Jedis publicJedis = null;
        try {
            publicJedisPool = DataBase.getJedisPublicPool();
            publicJedis = publicJedisPool.getResource();
            publicJedis.set(key, value);
        } catch (Exception e) {
            publicJedisPool.returnBrokenResource(publicJedis);
            throw new Exception("设置redis失败: key:" + key + ",value:" + value, e);
        } finally {
	...
	public static String getValue(String key) throws Exception {
		...
		try {
			...
			if (publicJedis != null)
				publicJedisPool.returnBrokenResource(publicJedis);
			throw new Exception("获取redis失败，key:" + key, e);
		} finally {
			if (publicJedis != null) {
				publicJedisPool.returnResource(publicJedis);
			}
