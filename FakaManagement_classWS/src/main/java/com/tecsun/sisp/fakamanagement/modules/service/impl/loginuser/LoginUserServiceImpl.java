...
public class LoginUserServiceImpl extends BaseService{
    ...
    //查询用户是否为银行角色
    public List<LoginPassswordVO> checkYhTypeByUserId(String userid){
        return this.getUserSqlSessionTemplate().selectList(packageName+"checkYhTypeByUserId", userid);
    }

    //查询用户是否为网点角色
    public List<LoginPassswordVO> checkWdTypeByUserId(String userid){
        return this.getUserSqlSessionTemplate().selectList(packageName+"checkWdTypeByUserId", userid);
    }
}
