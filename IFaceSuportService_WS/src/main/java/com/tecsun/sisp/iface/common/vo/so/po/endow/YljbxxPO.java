package com.tecsun.sisp.iface.common.vo.so.po.endow;


/**
 * 养老基本信息
 * @author liang
 */
public class YljbxxPO {
	private String GRBH;// 个人编号
	private String XZBM;// 险种编码
	private String XZMC;// 险种名称
	private String XM;// 姓名
	private String XB;// 性别名称
	private String MZ;// 民族名称
//	private String CSRQ;// 出生日期
	private String HKXZ;// 户口性质
	private String NL;// 年龄
	private String SFZH;// 身份证号码
	private String CJGZRQ;// 参加工作时间
	private String XL;// 学历
	private String CBRDH;// 参保人电话
	private String RYZT;// 人员状态
	private String CBZT;// 参保状态
	private String CBRSJ;// 参保人手机
	private String TXDZ;// 通讯地址
	private String YB;// 邮编
	private String HKSZDDZ;// 户口所在地地址
	private String DZYJDZ;// 电子邮件地址
	private String AIC162;//离退休日期
	
	
	public String getAIC162() {
		return AIC162;
	}
	public void setAIC162(String aIC162) {
		AIC162 = aIC162;
	}
//	public String getCSRQ() {
//		return CSRQ;
//	}
//	public void setCSRQ(String cSRQ) {
//		CSRQ = cSRQ;
//	}
	public String getCJGZRQ() {
		return CJGZRQ;
	}
	public void setCJGZRQ(String cJGZRQ) {
		CJGZRQ = cJGZRQ;
	}
	
	public String getGRBH() {
		return GRBH;
	}
	public void setGRBH(String gRBH) {
		GRBH = gRBH;
	}
	public String getXZBM() {
		return XZBM;
	}
	public void setXZBM(String xZBM) {
		XZBM = xZBM;
	}
	public String getXZMC() {
		return XZMC;
	}
	public void setXZMC(String xZMC) {
		XZMC = xZMC;
	}
	public String getXM() {
		return XM;
	}
	public void setXM(String xM) {
		XM = xM;
	}
	public String getXB() {
		return XB;
	}
	public void setXB(String xB) {
		XB = xB;
	}
	public String getMZ() {
		return MZ;
	}
	public void setMZ(String mZ) {
		MZ = mZ;
	}
	
	public String getHKXZ() {
		return HKXZ;
	}
	public void setHKXZ(String hKXZ) {
		HKXZ = hKXZ;
	}
	public String getNL() {
		return NL;
	}
	public void setNL(String nL) {
		NL = nL;
	}
	public String getSFZH() {
		return SFZH;
	}
	public void setSFZH(String sFZH) {
		SFZH = sFZH;
	}
	
	public String getXL() {
		return XL;
	}
	public void setXL(String xL) {
		XL = xL;
	}
	public String getCBRDH() {
		return CBRDH;
	}
	public void setCBRDH(String cBRDH) {
		CBRDH = cBRDH;
	}
	public String getRYZT() {
		return RYZT;
	}
	public void setRYZT(String rYZT) {
		RYZT = rYZT;
	}
	public String getCBZT() {
		return CBZT;
	}
	public void setCBZT(String cBZT) {
		CBZT = cBZT;
	}
	public String getCBRSJ() {
		return CBRSJ;
	}
	public void setCBRSJ(String cBRSJ) {
		CBRSJ = cBRSJ;
	}
	public String getTXDZ() {
		return TXDZ;
	}
	public void setTXDZ(String tXDZ) {
		TXDZ = tXDZ;
	}
	public String getYB() {
		return YB;
	}
	public void setYB(String yB) {
		YB = yB;
	}
	public String getHKSZDDZ() {
		return HKSZDDZ;
	}
	public void setHKSZDDZ(String hKSZDDZ) {
		HKSZDDZ = hKSZDDZ;
	}
	public String getDZYJDZ() {
		return DZYJDZ;
	}
	public void setDZYJDZ(String dZYJDZ) {
		DZYJDZ = dZYJDZ;
	}

	
}
